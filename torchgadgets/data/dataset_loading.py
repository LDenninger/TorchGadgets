...
def _load(dataset_name, mode=0):
    ...
    if dataset_name == 'cifar10':
        if mode == 2:
            ...
            return None
        dataset = datasets.CIFAR10(root='./data/cifar10', train=True if mode==0 else False, download=True)
    elif dataset_name == 'cifar100':
        if mode == 2:
            print(f'No validation dataset available for {dataset_name}')
            return None
        dataset = datasets.CIFAR100(root='./data/cifar100', train=True if mode==0 else False, download=True)
    elif dataset_name == 'mnist':
        if mode == 2:
            print(f'No validation dataset available for {dataset_name}')
            return None
        dataset = datasets.MNIST(root='./data/mnist', train=True if mode==0 else False, download=True)
    elif dataset_name == 'fashionmnist':
        if mode == 2:
            print(f'No validation dataset available for {dataset_name}')
            return None
        dataset = datasets.FashionMNIST(root='./data/fashionmnist', train=True, download=True)
        ...
        if mode == 0:
            ...
            s = 'test'
        dataset = datasets.SVHN(root='./data/svhn', split=s, download=True)
    elif dataset_name == 'oxfordpet':
        if mode == 2:
            print(f'No validation dataset available for {dataset_name}')
            return None
        dataset = datasets.OxfordIIITPet(root='./data/oxfordpet', split='trainval' if mode==0 else 'test', download=True)
    elif dataset_name == 'food101':
        if mode == 2:
            print(f'No validation dataset available for {dataset_name}')
            return None
        dataset = datasets.Food101(root='./data/food101', split='train' if mode==0 else 'test', download=True)
    if dataset is None:
